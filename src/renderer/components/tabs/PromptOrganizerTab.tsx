...
const APPEND_BUTTONS: Array<{ key: string; value: string }> = [
  ...
  { key: 'Update blocks', value: '\n---\nProvide the added, updated or deleted text/code in separate text/code blocks. Each block must start with a header stating the operation type (add, replace, delete) and the exact location in the file, followed by one line briefly explaining the reason for the change. Each existing, added, deleted and replacement block must be enclosed with backticks and content type (eg. text, python, terraform, etc.) as markdown code block. For partial replacements, both the existing text/code block and the corresponding replacement block need to be printed as 2 separate blocks with clear visual separation in between. If the operation is applied on a file in full, state this explicitly and do not include the existing file content. Do not place any symbols on individual lines alone, so every existing, added, deleted and replacement block can be copied to the clipboard directly.' },
  { key: 'Update blocks - conditional', value: '\n---\nIf changes are required, provide the added, updated or deleted text/code in separate text/code blocks. Each block must start with a header stating the operation type (add, replace, delete) and the exact location in the file, followed by one line briefly explaining the reason for the change. Each existing, added, deleted and replacement block must be enclosed with backticks and content type (eg. text, python, terraform, etc.) as markdown code block. For partial replacements, both the existing block and the corresponding replacement block need to be printed as 2 separate blocks with clear visual separation in between. If the operation is applied on a file in full, state this explicitly and do not include the existing file content. Do not place any symbols on individual lines alone, so every existing, added, deleted and replacement block can be copied to the clipboard directly.' },
];
