...
const APPEND_BUTTONS: Array<{ key: string; value: string }> = [
  ...
  { key: 'Update blocks', value: '\n---\nProvide the added, updated or deleted text/code in separate text/code blocks. Each block must start with a header stating the operation type (add, replace, delete) and the exact location in the file, followed by one line briefly explaining the reason for the change. Each existing, added, deleted and replacement block must be enclosed with backticks and content type (e.g. text, python, terraform, etc.) as markdown code block. For partial replacements, both the existing text/code block and the corresponding replacement block need to be printed as 2 separate blocks, with clear visual separation in between. An existing text/code block to be updated or a block to be deleted must include unique starting lines and ending lines from the file for me to locate the exact location of the block in the file, while the other lines can be ignored. A block to be added must indicate the exact position in the file after which I can add the new block in. If the operation is applied on a file in full, state this explicitly and do not include the existing file content. Do not place any symbols on individual lines alone, so every existing, added, deleted and replacement block can be copied to the clipboard directly.\n---\nPlease try to keep the proposed text/code changes minimal.' },
